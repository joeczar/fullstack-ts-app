...
describe('GET /users', () => {
    test('It should respond with an array of Users', async () => {
        const response = await supertest_1.default(app).get('/users');
...
describe('GET /users/:id', () => {
    test('It should respond with a single user and #NaN when given a bad id', async () => {
        const response = await supertest_1.default(app).get('/users/b');
    ...
    test('It should respond with a single user and the id when given an id', async () => {
        const response = await supertest_1.default(app).get('/users/5');
...
describe('POST /users', () => {
    test('it should respond with "Saving user...', async () => {
        const response = await supertest_1.default(app).post('/users');
...
describe('PUT /users/:id', () => {
    test('it should respond with "Saving user...', async () => {
        const response = await supertest_1.default(app).put('/users/5');
...
describe('DELETE /users/:id', () => {
    test('it should respond with "Removing user..."', async () => {
        const response = await supertest_1.default(app).delete('/users/5');
